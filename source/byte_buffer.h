...
namespace vx3d
{
    ...
    private:
        ...
        void _decompress()
        {
            ...
            do {
                stream.avail_in = glm::max(std::uint32_t(_size - compressed_index), CHUNK);
                if (stream.avail_in == 0) break;

                stream.next_in = reinterpret_cast<const uint8_t *>(&_data[compressed_index]);
    ...
    public:
        ...
        {
            auto bytes = std::array<std::uint16_t, 2>();
            std::generate(
              bytes.begin(),
              bytes.end(),
              [this]() { return static_cast<std::uint16_t>(read_u8()); });
            return _bswapu16(bytes[0] << 8 | bytes[1]);
        }
        ...
        {
            auto bytes = std::array<std::uint32_t, 4>();
            std::generate(
              bytes.begin(),
              bytes.end(),
              [this]() { return static_cast<std::uint32_t>(read_u8()); });
            return _bswapu32(bytes[0] << 24 | bytes[1] << 16 | bytes[2] << 8 | bytes[3]);
        }
        ...
        {
            auto bytes = std::array<std::uint64_t, 8>();
            std::generate(
              bytes.begin(),
              bytes.end(),
              [this]() { return static_cast<std::uint64_t>(read_u8()); });
            return _bswapu64(
